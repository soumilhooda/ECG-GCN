    ...
    def build_graph(self, M_0):
        ...
        with self.graph.as_default():
            ...
            with tf.name_scope('inputs'):
                self.ph_data = tf.keras.Input(shape=(M_0,), dtype=tf.float32, name='data')
                self.ph_labels = tf.keras.Input(shape=(), dtype=tf.int32, name='labels')
                self.ph_dropout = tf.keras.Input(shape=(), dtype=tf.float32, name='dropout')
...
#        assert len(L) >= 1 + np.sum(p_log2)  # Enough coarsening levels for pool sizes.
    ...
    def chebyshev5(self, x, L, Fout, K):
        # Ensure the input dimensions are fully defined
        N, M, Fin = x.get_shape().as_list()
        if N is None or M is None or Fin is None:
            raise ValueError("The dimensions of the input tensor 'x' must be fully defined. "
                            "Received dimensions: N={}, M={}, Fin={}".format(N, M, Fin))
        N, M, Fin = int(N), int(M), int(Fin)
